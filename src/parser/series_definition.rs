...
fn label() -> impl Parser<char, Spanned<String>, Error = Simple<char>> {
    class().map_with_span(|c, span| (span, c.to_string()))
}

pub fn parser() -> impl Parser<char, Stmt, Error = Simple<char>> {
    let start = just("series").padded();
    ...
    let category = category().map(Series::Category);

    let body =
        choice([list.boxed(), category.boxed()]).map_with_span(|series, span| (span, series));
