...
use crate::ast::{
    EnvElement, EnvPattern, Environment, Pattern, Segment, Source, Spanned, Stmt, Target,
};

fn segment() -> impl Parser<char, Segment, Error = Simple<char>> {
    choice([
        word_chars().map(Segment::Phonemes).boxed(),
        category().map(Segment::Category).boxed(),
    ])
}
...
pub fn parser() -> impl Parser<char, Stmt, Error = Simple<char>> {
    ...
    let target = just(">").padded().ignore_then(target());

    let environment = just("/").padded().ignore_then(environment()).or_not();

    let description = just(":")
        .padded()
        .ignore_then(description().map_with_span(|desc, span| (span, desc)))
        .or_not();
    ...
    start
        ...
        .then(description)
        .map(
            |(((source, target), environment), description)| Stmt::SoundChange {
                source,
                target,
                environment,
                description,
        ...
        assert_eq!(
            ...
            Ok(Stmt::SoundChange {
                ...
                environment: Some((
                    ...
                    Environment {
                        before: None,
                        after: Some(vec![EnvElement::Segment(Segment::Category(Category {
    ...
    fn it_parses_a_sound_change_with_categories_and_modifications() {
        ...
        assert_eq!(
            ...
            Ok(Stmt::SoundChange {
                ...
                target: (
                    22..29,
                    Target::Modification(vec![(23..28, Feature::Positive("flap".to_string())),])
                ),
                environment: None,
                description: None,
            })
